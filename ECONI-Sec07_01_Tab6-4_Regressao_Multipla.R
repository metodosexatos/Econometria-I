...
#************* Módulo 07 - Exemplo Ilustrativo Modelo de Regressão Linear Múltipla  *************#
...
base <- read.csv2("Tab_6-4_mortalidade_infantil.csv") # carrega base de dados
str(base) # ver a estrutura da base

# Função de Regressão Amostral
FRA <- lm(MI~PNBpc+TAF, data = base) # modelo de regressão amostral (para excluir o intercepto colocar "-1" depois da última variável explanatória:"MI~PNBpc+TAF-1" )
...
scatter3d(MI~PNBpc+TAF, data = base)
