...
FRA <- lm(Y~X) # modelo de regressão linear
summary(FRA)   # resumo estatístico do modelo

# Coeficientes

install.packages("broom") # pacote para facilitar a extração de informações do FRA
library("broom")
...
estatistica_t <- t$statistic[1]                    # armazena apenas o resultado da estatística t
estatistica_t                                      # exibe o valor da estatística t

# Intervalo de Confiança (IC)
#- IC beta 1
li_beta1 <- beta1 - estatistica_t*erro_beta1       # calcula o limite inferior de controle (LIC) para beta 1 e armazena na respectiva variável
li_beta1                                           # exibe o LIC para beta 1
ls_beta1 <- beta1 + estatistica_t*erro_beta1       # calcula o limite superior de controle (LSC) para beta 1 e armazena na respectiva variável
ls_beta1                                           # exibe o LSC para beta 1

#- IC beta 2
li_beta2 <- beta2 - estatistica_t*erro_beta2       # calcula o limite inferior de controle (LIC) para beta 2 e armazena na respectiva variável
li_beta2                                           # exibe o LIC para beta 2
ls_beta2 <- beta2 + estatistica_t*erro_beta2       # calcula o limite superior de controle (LSC) para beta 2 e armazena na respectiva variável
ls_beta2                                           # exibe o LSC para beta 2

#- IC da variância
alfa <- 0.05                                            # nível de significância
gl <- df.residual(FRA)                                  # graus de liberdade
qq_a2 <- qchisq(alfa/2, lower.tail = FALSE, df = gl)    # valor crítico X² para alfa/2
qq_1a2 <- qchisq(alfa/2, lower.tail = TRUE, df = gl)    # valor crítico X² para 1-alfa/2

EY <- beta1+beta2*X   # valores estimados de Y: E(Y|X)
EY                    # exibe os valores estimados de Y

Subt_Y <- Y-EY          # resíduos
Subt_Y                  # exibe os resíduos
sy2 <- Subt_Y^2         # resíduos elevados ao quadrado
sy2                     # exibe os resíduos elevados ao quadrado
soma_y2 <- sum(sy2)     # soma dos resíduos elevados ao quadrado
soma_y2                 # exibe a soma dos resíduos elevados ao quadrado
var_fra<- soma_y2/gl    # determina a variância do modelo
var_fra                 # exibe a variância do modelo

li_var <- gl*var_fra/qq_a2    # LIC para variância
li_var                        # exibe o LIC para variância
ls_var <- gl*var_fra/qq_1a2   # LSC para variância
ls_var                        # exibe o LSC para variância
